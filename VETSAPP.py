...
def fetch_consultations_of_customer_pets(id):
    ...
    return render_template('consultations-pets.html',
                           ...
                           pet=pet,
                           documents=documents)
@web_app.route("/search-customer-before-update/<id>")
def search_customer_before_update(id):
    db = MongoDBHelper(collection="customer")
    query = {'_id': ObjectId(id), 'vet_id': session['vet_id']}
...
def update_consultations(id):
    ...
    return render_template('update-consultation.html', consultation=consultation)
...
def update_consultation_in_db():
    ...
    return render_template('success.html', message="{} updated successfully".format(consultation_data_to_update['problem']))

@web_app.route("/delete-consultation/<id>")
def delete_pet_consultation(id):
    db = MongoDBHelper(collection="consultation")
    query = {'_id': ObjectId(id)}
    consultation = db.fetch(query)[0]
    db.delete(query)
    return render_template('success.html', message="Pet's Consultation {} Deleted".format(consultation['problem']))
# To search Customer
